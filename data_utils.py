...
def download_dataset(dataset, files, data_dir):
    ...
    if not np.all([os.path.isfile(data_dir + f) for f in files]):
        ...
        if dataset in ['ml_100k', 'ml_1m']:
            target_dir = 'ALP/raw_data/' + dataset.replace('_', '-')
        elif dataset == 'ml_10m':
            target_dir = 'ALP/raw_data/' + 'ml-10M100K'
        else:
...
def load_movielens_features(fname, seed=1234, verbose=True):
    u_features = None
    v_features = None
    ...
    if fname == 'ml_100k':
        ...
        # make sure to convert to list, otherwise random.shuffle acts weird on it
        # without a warning
        ...
        u_nodes_ratings, v_nodes_ratings = u_nodes_ratings.astype(np.int64), \
        ...
        # use engine='python' to ignore warning about switching to python backend
        # when using regexp for sep
    ...
    return num_users, num_items, u_nodes_ratings, v_nodes_ratings, \
